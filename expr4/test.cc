...
void test ()
{
  ...
  //Added due to limitation of #5666
  ...
  //This has a problem though - see #5666
  //So, here's a workaround

  int num_of_blocks = 2;

  std::vector<types::global_dof_index> dofs_per_block (num_of_blocks);
  //DoFTools::count_dofs_per_component (dof_handler, dofs_per_block);
  ...
  {
    BlockDynamicSparsityPattern csp (num_of_blocks,num_of_blocks);

    for (unsigned int d=0; d<num_of_blocks; ++d)
      for (unsigned int e=0; e<num_of_blocks; ++e)
        csp.block(d,e).reinit (dofs_per_block[d], dofs_per_block[e]);
  ...
  solution.reinit (num_of_blocks);
  vec1.resize (num_of_blocks);
  vec2.resize (num_of_blocks);
  ...
  // first system_rhs with random numbers
  for (unsigned int i=0; i<num_of_blocks; ++i) //loop on blocks
