/*
 * this case will serve as a perf benchmark test for the implementation of matrix
 * free operators developed for anisotropic Finite Elements.
 * This case uses the new implementation and applies it on FE_Q vector
 * valued elements (Q(k+1),Qk. The operators chosen are from Darcy porous media
 * flow problem with unit permeability tensor. As such the chosen Finite Element
 * has no utility for diffusion problem, and this is also not the purpose.
 ...
 */
//This test should not be added to dealii tests

/*
 * There are quite some changes which have to be done compared with the
 * implementation for RT elements:
 * 1. Define fe_u as object of FESystem
 * 2. Use FE_Q for pressure space
 * 3. Reorder the DOFs to something which MF framework expects when input
 *    is a BlockVector
 * 4. After operation evaluation, reorder back for comparison with dealii
 *
 */
...
#include <deal.II/matrix_free/fe_evaluation.h>
  ...
  public:
    ...
    MF_Stokes(const MatrixFree<dim,Number> &data_in):
    	data (data_in)
    {};
                ...
                const VectorType &src) const
    {
    	dst.block(0) = 0;
    	dst.block(1) = 0;
    	data.cell_loop (&MF_Stokes<dim,degree_p,VectorType>::local_apply_opA,
                    this, dst, src);
    };
                ...
                const VectorType &src) const
    {
    	dst.block(0) = 0;
    	dst.block(1) = 0;
    	data.cell_loop (&MF_Stokes<dim,degree_p,VectorType>::local_apply_opB,
                    this, dst, src);
                ...
                const VectorType &src) const
    {
    	dst.block(0) = 0;
    	dst.block(1) = 0;
    	data.cell_loop (&MF_Stokes<dim,degree_p,VectorType>::local_apply_opB_tr,
                    this, dst, src);
  ...
  template <int dim, int degree>
  TestStokes<dim, degree>::TestStokes ()
    :
    n_q_points_1d(degree+2),
    fe (FE_Q<dim>(degree+1), dim,FE_Q<dim>(degree),1), //Taylor-Hood
    dof_handler (triangulation),
    //For MF
    fe_u(FE_Q<dim>(degree+1), dim),
    fe_p(FE_Q<dim>(degree)),
  ...
  template <int dim, int degree>
  void TestStokes<dim,degree>::make_grid_and_dofs ()
  {
...
//#endif
  ...
  template <int dim, int degree>
  void TestStokes<dim,degree>::assemble_sparse_system ()
  {
    ...
    //For MF using Blockvector, we need to Reorder system_rhs.block(0) and store into mf_rhs.
    // This is how MF expects input in BlockVector
    const unsigned int n_u = dof_handler_u.n_dofs();
    for (int i=0; i<n_u/dim;i++)
    {
    	mf_rhs.block(0)(i*dim) = system_rhs.block(0)(i);
    	mf_rhs.block(0)(i*dim+1) = system_rhs.block(0)(n_u/dim+i);
    }
    mf_rhs.block(1) = system_rhs.block(1); //Pressure is single component, so keep as-it-is
  }

  template <int dim, int degree>
  void TestStokes<dim,degree>::compute_errors (int block_no) const
  {
	        // Verification
	        double error = 0.;
	        for (unsigned int i=0; i<2; ++i)
  ...
  template <int dim, int degree>
  void TestStokes<dim,degree>::test_mf_operator (
		  	  	  	  	  	  	  	  enum op_id id,Timer &time,
		  	  	  	  	  	  	  	  BlockSparseMatrix<double> &tmp_system_matrix)
  {
	  //vmult using MF
      typedef  BlockVector<double> VectorType;
      MF_Stokes<dim,degree,VectorType> mf (mf_data);
      BlockVector<double> tmp_mf_solution(mf_solution);
	  ...
	  if (OP_A == id)
	  {
		  ...
		  time.restart();
		  mf.Avmult(tmp_mf_solution, mf_rhs);
	  }
	  ...
	  if (OP_B == id)
	  {
		  ...
		  tmp_system_matrix.block(0,1) = 0;
		  time.restart();
		  mf.Bvmult(tmp_mf_solution, mf_rhs);
	  }
	  ...
	  if (OP_B_tra == id) //We test Au + B_tr.p
	  {
		  tmp_system_matrix.block(1,0) = 0;
		  time.restart();
		  mf.B_tra_vmult(tmp_mf_solution, mf_rhs);
	  }

	  //Reorder back the solution vector. We consider also this operation as time required for MF operation
	  const unsigned int n_u = dof_handler_u.n_dofs();
	  for (int i=0; i<n_u/dim;i++)
	  {
		  mf_solution.block(0)(i) = tmp_mf_solution.block(0)(i*dim);
		  mf_solution.block(0)(n_u/dim+i) = tmp_mf_solution.block(0)(i*dim+1);
	  }
	  mf_solution.block(1) = tmp_mf_solution.block(1);
  ...
  template <int dim, int degree>
  void TestStokes<dim,degree>::run ()
  {
	Timer time;
  ...
  try
    {
	  ...
	  deallog.push("2d");
      TestStokes<2, 1> test1; test1.run(); //ok
      TestStokes<2, 2> test2; test2.run(); //ok
      TestStokes<2, 3> test3; test3.run(); //ok
      TestStokes<2, 4> test4; test4.run(); //ok
      TestStokes<2, 5> test5; test5.run(); //ok
      deallog.pop();
    }
